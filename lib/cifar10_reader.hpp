 ...
 * \return The dataset
 */
template <template <typename...> class Container = std::vector, template <typename...> class Array = std::vector, typename Pixel = uint8_t, typename Label = uint8_t>
CIFAR10_dataset<Container, Array<Pixel>, Label> read_dataset(std::size_t training_limit = 0, std::size_t test_limit = 0) {
    return read_dataset_direct<Container, Array<Pixel>, Label>(training_limit, test_limit);
}
